...
RuntimeInterface$jscomp$1.AddDOMHandlerClass(class extends self.DOMHandler {
    ...
    Attach() {
        ...
        window.addEventListener("click", a => {
            console.log(a); // Log the 'a' event object
            // Your other click event handling logic here
        }, !0); // true for capturing phase, if needed
...
function StopPropagation$jscomp$1(a) {
    a.stopPropagation()
}
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMElementHandler {
    constructor(a) {
    ...
    CreateElement(a, b) {
        ...
        c.addEventListener("keydown", StopPropagation$jscomp$1);
        c.addEventListener("keyup", StopPropagation$jscomp$1);
        c.addEventListener("click", () => this._PostToRuntimeElementMaybeSync("click", a));
        c.addEventListener("change", () => this.PostToRuntimeElement("change", a, {
            value: parseFloat(c.value)
        }));
        c.addEventListener("input", () => this.PostToRuntimeElement("input", a, {
            value: parseFloat(c.value)
        }));
        b.id &&
            (c.id = b.id);
...
function ClearElement(a) {
    a.replaceChildren ? a.replaceChildren() : a.innerHTML = ""
}

function SetHTMLContent(a, b) {
    a.innerHTML = b;
    ExecuteInsertedScripts(a)
}

function InsertAdjacentHTML(a, b, c) {
    a.insertAdjacentHTML(b, c);
    ExecuteInsertedScripts(a)
}

function ExecuteInsertedScripts(a) {
    a = Array.from(a.querySelectorAll("script:not([c3-executed])"));
    for (const b of a) b.parentNode.replaceChild(CloneScriptNode(b), b)
}

function CloneScriptNode(a) {
    const b = document.createElement("script");
    b.text = a.text;
    a = a.attributes;
    ...
    b.setAttribute("c3-executed", "");
    return b
}

function elemsForSelector$jscomp$1(a, b, c) {
    return b ? c ? Array.from(a.querySelectorAll(b)) : (a = a.querySelector(b)) ? [a] : [] : [a]
}

function HTMLUpdateResult(a) {
    return a ? {
        isOk: !0,
        html: a.innerHTML,
        text: a.textContent
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMElementHandler {
    constructor(a) {
        ...
        this.AddDOMElementMessageHandler("set-content", (b, c) => this._OnSetContent(b, c));
        this.AddDOMElementMessageHandler("insert-content", (b, c) => this._OnInsertContent(b, c));
        this.AddDOMElementMessageHandler("remove-content", (b, c) => this._OnRemoveContent(b, c));
        this.AddDOMElementMessageHandler("set-content-class", (b, c) => this._OnSetContentClass(b, c));
        this.AddDOMElementMessageHandler("set-content-attribute",
            (b, c) => this._OnSetContentAttribute(b, c));
        this.AddDOMElementMessageHandler("set-content-css-style", (b, c) => this._OnSetContentCSSStyle(b, c));
        this.AddDOMElementMessageHandler("get-element-box", (b, c) => this._OnGetElementBox(b, c));
        this.AddDOMElementMessageHandler("insert-img-element", (b, c) => this._OnInsertImgElement(b, c));
        this.AddDOMElementMessageHandler("set-scroll-position", (b, c) => this._OnSetScrollPosition(b, c))
    }
    CreateElement(a, b) {
        const c = document.createElement(b.tag);
        b["style-attribute"] && c.setAttribute("style",
            b["style-attribute"]);
        c.style.position = "absolute";
        var d = b["stop-input-events-mode"];
        if (0 < d) {
            let e = null;
            1 === d ? e = f => {
                f.target !== c && f.stopPropagation()
            } : 2 === d && (e = f => f.stopPropagation());
        ...
        b["allow-context-menu"] && c.addEventListener("contextmenu", e => e.stopPropagation());
        c.addEventListener("click", e => {
            console.log(e);
            e = e.target;
            const f = [];
            for (; e;) {
                f.push({
                    targetId: "string" === typeof e.id ? e.id : "",
                    targetClass: "string" === typeof e.className ? e.className : ""
                });
                if (e === c) break;
                e = e.parentNode
            }
        ...
        c.addEventListener("animationend", e => {
            const f = e.target;
            this._PostToRuntimeElementMaybeSync("animationend", a, {
                targetId: "string" === typeof f.id ? f.id : "",
                targetClass: "string" === typeof f.className ? f.className : "",
        ...
        b.id &&
            (c.id = b.id);
        b.className && (c.className = b.className);
        b["css-color"] && (c.style.color = b["css-color"]);
        b["css-background-color"] && (c.style.backgroundColor = b["css-background-color"]);
        b["allow-text-selection"] || (c.style.userSelect = "none", c.style.webkitUserSelect = "none");
        d = b.str;
        "html" === b.type ? SetHTMLContent(c, d) : c.textContent = d;
    ...
    _OnSetContent(a, b) {
        ...
        b = b["is-all"];
        try {
            const f = elemsForSelector$jscomp$1(a, e, b);
            if (0 === f.length) return HTMLUpdateResult(null);
            for (const g of f) "html" === d ? SetHTMLContent(g, c) : g.textContent = c;
            return HTMLUpdateResult(a)
        } catch (f) {
            return console.warn("[HTML element] Failed to set content: ", f), HTMLUpdateResult(null)
        }
    }
    _OnInsertContent(a, b) {
        const c = b.str,
            d = b.type,
            e = b["at-end"] ? "beforeend" : "afterbegin",
            f = b.selector;
        b = b["is-all"];
        try {
            const g = elemsForSelector$jscomp$1(a, f, b);
            if (0 === g.length) return HTMLUpdateResult(null);
            for (const h of g) "html" === d ? InsertAdjacentHTML(h, e, c) : h.insertAdjacentText(e, c);
            return HTMLUpdateResult(a)
        } catch (g) {
            return console.warn("[HTML element] Failed to insert content: ", g), HTMLUpdateResult(null)
        }
    }
    _OnRemoveContent(a, b) {
        const c = b["is-clear"],
            d = b.selector;
        b = b["is-all"];
        if (!d) return ClearElement(a), HTMLUpdateResult(a);
        try {
            const e = elemsForSelector$jscomp$1(a, d, b);
            if (0 === e.length) return HTMLUpdateResult(null);
            for (const f of e) c ? ClearElement(f) : f.remove();
            return HTMLUpdateResult(a)
        } catch (e) {
    ...
    _OnSetContentClass(a, b) {
        ...
        try {
            ...
            if (0 === f.length) return HTMLUpdateResult(null);
            for (const g of f)
                for (const h of d) switch (c) {
                    case "add":
                        g.classList.add(h);
                        break;
                    case "toggle":
                        g.classList.toggle(h);
                        break;
                    case "remove":
            ...
            return console.warn("[HTML element] Failed to set class: ", f), HTMLUpdateResult(null)
        }
    }
    _OnSetContentAttribute(a,
        b) {
        const c = b.mode,
            d = b.attribute,
            e = b.value,
        ...
        try {
            ...
            return console.warn("[HTML element] Failed to set attribute: ", g), HTMLUpdateResult(null)
        }
    }
    _OnSetContentCSSStyle(a, b) {
        const c = b.prop,
            d = b.value,
            e = b.selector;
        b = b["is-all"];
        try {
            const f = elemsForSelector$jscomp$1(a,
                e, b);
            if (0 === f.length) return HTMLUpdateResult(null);
            for (const g of f) c.startsWith("--") ? g.style.setProperty(c, d) : g.style[c] = d;
            return HTMLUpdateResult(a)
        } catch (f) {
            return console.warn("[HTML element] Failed to set style: ", f), HTMLUpdateResult(null)
        }
    }
    _OnGetElementBox(a, b) {
        a = (b = b.selector) ? a.querySelector(b) : a;
        if (!a) return {
            isOk: !1
        };
    ...
    _OnInsertImgElement(a, b) {
        ...
        const e = b.height,
            ...
            g = b.insertAt;
        let h = a;
        if (f && (h = a.querySelector(f), !h)) return {
            isOk: !1
        };
        d = new Image(d, e);
        d.src = c;
        b.id && (d.id = b.id);
        b["class"] && (d.className = b["class"]);
        0 === g ? h.prepend(d) : 1 === g ? h.append(d) : h.replaceChildren ? h.replaceChildren(d) : (ClearElement(h), h.append(d));
        return HTMLUpdateResult(a)
    }
    _OnSetScrollPosition(a, b) {
        const c = b.selector,
            d = b.direction;
        b = b.position;
        if (c && (a = a.querySelector(c), !a)) return;
        "left" === d ? a.scrollLeft = b : a.scrollTop = b
    }
});